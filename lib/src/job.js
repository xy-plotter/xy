...
function Job (jobName, config) {
  ...
  const api = {
    ...
    setSpeed: (speedPercent) => {
      if (speedPercent) {
        if (speedPercent < 0 || speedPercent > 1) {
          sh.warn(`Speed value must be between 0 and 1. Leaving speed setting unchanged.`)
    ...
    wait: (promise) => {
      function waitForIt () {
        return sh.input('Waiting... press enter to continue.', () => { return true })
      }
    ...
    move: (x, y) => {
      ...
      if (x < 0 || x > config.width || y < 0 || y > config.height) {
        if (!alreadyWarned) {
          alreadyWarned = true
          sh.warn(`Some positions will be outside plotter's boundaries. [${x}, ${y}]`)
        }
      }
    ...
    svg: (file, _transformation) => {
      ...
      } else sh.warn(`${file} doesn't contain any valid points.`)
      return api
    },
    ...
    text: (string, x, y, opts) => {
      let points = SVG.fromText(string, x, y, opts)
      for (let i = 0; i < points.length; i++) {
        let point = points[i]
        if (point[0] === 'M') {
          api.pen_up()
