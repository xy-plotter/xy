...
function Stats (job, config) {
  let cmds = parse(job.buffer)
  let distance = calcDistance(cmds)
  let duration = calcDuration(cmds)
  ...
  const api = {
    ...
    refresh: () => {
      cmds = parse(job.buffer)
      distance = calcDistance(cmds)
      duration = calcDuration(cmds)
      return api
    },
    ...
    pretty: (emoji = true) => {
      return `${emoji ? '❓  ' : ''}${job.name}` + '\n' +
             `${emoji ? '✏️  ' : ''}${distance} mm` + '\n' +
             `${emoji ? '⏳  ' : ''}${duration.estimation.formatted} (${duration.min.formatted} ~ ${duration.max.formatted})`
    }
  }
